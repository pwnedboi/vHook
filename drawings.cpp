...
HFONT framefont, espfont, mFont, osFont, sFont, tFont, cFont, titleFont, subtitleFont, heading, o4Font_20, o4Font_11, o4Font_03;
...
void cdrawings::drawCrosshair1(int x, int y, Color col) {
	this->drawline(x - 10, y, x + 10, y, col);
	this->drawline(x, y - 10, x, y + 10, col);
}
void cdrawings::drawCrosshair2(int x, int y, Color col) {
	this->drawline(x - 4, y - 4, x + 4, y + 4, col);
	this->drawline(x + 4, y - 4, x - 4, y + 4, col);
	this->drawline(x - 2, y - 2, x + 2, y + 2, col);
	this->drawline(x + 2, y - 2, x - 2, y + 2, col);
}
void cdrawings::drawCrosshair3(int x, int y, Color col) {
	this->drawbox(x - 5, y - 5, 10, 10, col);
}
...
void cdrawings::addslider(int x, int y, int w, const char* szString, int& value, int index, int max, int min) {
	...
	string factor = to_string(value);
	string tostring = szString;
	
	string tofinal = tostring + ": " + factor;
...
void cdrawings::DrawBasicString(int x, int y, Color color, HFONT font, string szString) {
	wstring wString = StringToWstring(szString);
	pSurface->DrawSetTextPos(x, y);
	pSurface->DrawSetTextFont(font);
	pSurface->DrawSetTextColor(color);
	pSurface->DrawPrintText(wString.c_str(), wcslen(wString.c_str()));
...
void cdrawings::Checkbox(int x, int y, const char* szValue, bool* var) {
	
	int size = 20;
	
	if (*var)
		draw->fillrgba( x + 2, y + 2, size - 4, size - 4, Color( 20, 70, 145, 255 ) );
	
	
	if (inArea(x, y, 20, 20)) {
		if (*var)
			this->fillrgba( x + 2, y + 2, size - 4, size - 4, Color( 30, 80, 155, 255 ) );
		else
			this->fillrgba( x + 2, y + 2, size - 4, size - 4, Color( 60, 60, 135, 255 ) );
		
		if (bWasReleased)
			*var = !*var;
	}
	
	this->drawbox(x, y, 20, 20, Color(90, 90, 90, 230));
	
	this->drawstring(x + size + 5, y + 2, Color(255, 255, 255, 150), cFont, szValue);
	
	
}

void cdrawings::Slider(int x, int y, int w, const char* szString, int& value, int max, int min) {
	...
	string cunt = tostring + " : " + factor;
	
	if(w < 180 || w > 180)
		w = 180;
	
	this->fillrgba( x, y + 5, w + 2 , 7, Color( 42, 42, 42, 255 ) ); // background
	this->fillrgba( x, y + 5, isCurrent, 7, Color( 25, 25, 25, 255 ) ); // Color till cursor
	this->drawboxoutline( x, y + 5, w + 2, 7, Color( 90, 90, 90, 255 ) );
	this->drawstring( x, y - 16, Color(255, 255, 255, 100), cFont, cunt.c_str());
	
	// Slider
	this->fillrgba( x + (isCurrent), y + 2, 5, 13, Color( 25, 25, 25, 255 ) ); //Cursor // Sliding box / Slider
	this->drawboxoutline( x + (isCurrent), y + 2, 5, 13, Color( 90, 90, 90, 255 ) ); //Cursor border, Sexy <3
	
}

void cdrawings::Slider(int x, int y, int w, const char* szString, float& value, int max, int min) {
...
void cdrawings::RenderComboBox(int x, int y, int w, int h, const char* szString, vector<string> szValue, int& currValue, bool* bOpend) {
	...
	if (*bOpend) {
		...
		for ( int i = 0; i < szValue.size(); i++ ) {
			...
			auto bHover = inArea(x, Y + 2 + ( i * 20 ), w, 18);
			
			this->fillrgba( x, Y + 2 + ( i * 20 ), w, 18, bHover ? Color( 150, 20, 20, 230 ) : Color( 50, 50, 50, 230 ) );
			this->drawboxoutline( x, Y + 2 + ( i * 20 ), w, 18, Color( 90, 90, 90, 255 ) );
			this->drawstring(x + 5, Y + 3 + (i*20), Color::White(), cFont, szValue[i].c_str());
			
			if (inArea(x, Y + 2 + ( i * 20 ), w, 18)) {
				if(bWasPressed){
					currValue = i;
					*bOpend = false;
				}
	...
	this->fillrgba( x + w - h, y, h, h, Color( 30, 80, 155, 255 ) );
	this->drawboxoutline( x, y, w, h, Color( 90, 90, 90, 255 ) );
	this->drawstring(x + 5, y + (h/2) - 7, Color(255, 255, 255, 100), cFont, szString);
	
	
	if (!*bOpend)
	{
		// draw arrow pointing down
		this->fillrgba( ( x + w - h + 113 - 107 ), ( y + 8 ), 1, 1, Color( 255, 255, 255, 255 ) );
		this->fillrgba( ( x + w - h + 114 - 107 ), ( y + 8 ), 1, 2, Color( 255, 255, 255, 255 ) );
		this->fillrgba( ( x + w - h + 115 - 107 ), ( y + 8 ), 1, 3, Color( 255, 255, 255, 255 ) );
		this->fillrgba( ( x + w - h + 116 - 107 ), ( y + 8 ), 1, 4, Color( 255, 255, 255, 255 ) );
		this->fillrgba( ( x + w - h + 117 - 107 ), ( y + 8 ), 1, 3, Color( 255, 255, 255, 255 ) );
		this->fillrgba( ( x + w - h + 118 - 107 ), ( y + 8 ), 1, 2, Color( 255, 255, 255, 255 ) );
		this->fillrgba( ( x + w - h + 119 - 107 ), ( y + 8 ), 1, 1, Color( 255, 255, 255, 255 ) );
	}
	else
	{
		// draw arrow pointing up
		this->fillrgba( ( x + w - h + 113 - 107 ), ( y + 11 ), 1, 1, Color( 255, 255, 255, 255 ) );
		this->fillrgba( ( x + w - h + 114 - 107 ), ( y + 10 ), 1, 2, Color( 255, 255, 255, 255 ) );
		this->fillrgba( ( x + w - h + 115 - 107 ), ( y + 9 ), 1, 3, Color( 255, 255, 255, 255 ) );
		this->fillrgba( ( x + w - h + 116 - 107 ), ( y + 8 ), 1, 4, Color( 255, 255, 255, 255 ) );
		this->fillrgba( ( x + w - h + 117 - 107 ), ( y + 9 ), 1, 3, Color( 255, 255, 255, 255 ) );
		this->fillrgba( ( x + w - h + 118 - 107 ), ( y + 10 ), 1, 2, Color( 255, 255, 255, 255 ) );
		this->fillrgba( ( x + w - h + 119 - 107 ), ( y + 11 ), 1, 1, Color( 255, 255, 255, 255 ) );
	}
}
...
void cdrawings::addcombo(int x, int y, int w, const char* szString, vector<string> szValue, int& currValue, bool* bOpend, int index) {
	y += index * vars.buttonPadding;
	int h = 15;
...
void cdrawings::textbox(int x, int y, int w, const char* szString, string szValue[100], int inputIndex, int index, bool* var) {
	...
	if(szValue[inputIndex].length() > 0) {
		this->drawstring(x + 15, y + 1, Color(160, 160, 160, 255), cFont, szValue[inputIndex].c_str());
	}
	
	
	
	this->drawstring(x + w + 3, y  + 1, Color(160, 160, 160, 255), cFont, szString);
...
bool bCheckState() {
    return (pEngine->IsInGame() && pEngine->IsConnected());
}
...
void cdrawings::drawmouse() { // Draws mouse
	...
	for(int i = 0; i < 4; i++)
		this->fillrgba(mouse_x + 2 + i, mouse_y + 2 + i, 1, 14 - (i * 2), Color(220, 220, 220, 230));
	this->fillrgba(mouse_x + 6, mouse_y + 6, 1, 6, Color(220, 220, 220, 230));
	this->fillrgba(mouse_x + 7, mouse_y + 7, 1, 5, Color(220, 220, 220, 230));
	this->fillrgba(mouse_x + 8, mouse_y + 8, 1, 4, Color(220, 220, 220, 230));
	this->fillrgba(mouse_x + 9, mouse_y + 9, 1, 3, Color(220, 220, 220, 230));
	this->fillrgba(mouse_x + 10, mouse_y + 10, 1, 2, Color(220, 220, 220, 230));
	this->fillrgba(mouse_x + 11, mouse_y + 11, 1, 1, Color(220, 220, 220, 230));
}
