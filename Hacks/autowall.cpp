...
Vector CalcuAngle(Vector src, Vector dst) {
    Vector angles;
...
void ScaleDamage(HitGroups hitgroup, C_BaseEntity* enemy, float weapon_armor_ratio, float &current_damage)
{
...
bool TraceToExit(Vector &end, trace_t *enter_trace, Vector start, Vector dir, trace_t *exit_trace)
{
    ...
    while (distance <= vars.aimbot.fov)
    {
        distance += 4.0f; // 4.0f
...
float GetDamage(const Vector& point, bool teamCheck, FireBulletData& fData)
{
    ...
    Vector angles = CalcuAngle(data.src, dst);
...
float GetDamage(C_BaseEntity* pLocal, Vector HitboxPosition) {
    float damage = 0.f;
    ...
    if (SimulateFireBullet(activeWeapon, true, data))
        damage = data.current_damage;
